	...
	public class ShopMain : MonoBehaviour
	{
        ...
        private void SetBalance(CommandArgs args)
        {
            ...
            if (user != null)
            {
                ...
                EconomyCore.wallets[user.steamid] = amount;
                EconomyCore.SaveWallets();
            }
            else
            {
                for (int i = 0; i < args.Parameters[0].Length; i++)
                {
                    ...
                    {
                        Reference.Tell(args.sender.networkPlayer, $"Parametr length: {args.Parameters[0].Length}");
                ...
                if (ParameterIsSteamId)
                {
                    ...
                    EconomyCore.wallets[args.Parameters[0]] = amount;
                    EconomyCore.SaveWallets();
                }
                else
                {
        ...
        private void Pay(CommandArgs args)
        {
            ...
            if (user != null)
            {
                float receiveramount = EconomyCore.GetBalance(user.steamid);
                EconomyCore.wallets[user.steamid] += amount;
                EconomyCore.wallets[args.sender.steamid] -= amount;
        ...
        private void Cost(CommandArgs args)
        {
            ...
            float price = EconomyCore.Cost(args.Parameters[0]);
            if(price == -1)
            {
        ...
        private void Sell(CommandArgs args)
        {
            ...
            for (int i = 0; i < pinventory.clientItem_0.GetLength(0); i++)
            {
                for (int j = 0; j < pinventory.clientItem_0.GetLength(1); j++)
                {
                    try
                    {
                        ...
                        {
                            itemAmountInSeller+=slot.Int32_1;
                            if(itemAmountInSeller > itemAmount)
                            {
                                args.sender.player.networkView.RPC("tellItemSlot", RPCMode.All, new object[] { i, j, Convert.ToInt32(itemID), (itemAmountInSeller-itemAmount), ItemState.getState(Convert.ToInt32(itemID)) });
                            }
                            else
                                args.sender.player.networkView.RPC("tellItemSlot", RPCMode.All, new object[] { i, j, -1, 0, string.Empty });
            ...
            switch (ServerAnswer)
            {
                ...
                case -2:
                    ...
                    return;
                case 0:
                    Reference.Tell(args.sender.networkPlayer, $"Successfully selled {itemAmount}x Items with ID {itemID}, you earned {EconomyCore.Cost(itemID)*itemAmount*EconomyCore.SellIndex} {EconomyCore.CoinSymbol}");
                    break;
                default:
                    Reference.Tell(args.sender.networkPlayer, "Unknown error, try again.");
                    return;
